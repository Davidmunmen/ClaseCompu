...
int main() {
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		view = glm::translate(view, glm::vec3(3.0f,0.0f,-10.0f));
		model = glm::rotate( model, 0.5f, glm::vec3( -6.0f, -2.0f, -5.0f ) ); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(1.0f, 1.10f, 1.0f));
		...
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-10.0f, -1.0f, -8.0f));
		model = glm::rotate(model, 0.45f, glm::vec3(-100.0f, 220.0f, 360.0f)); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(4.0f, 0.10f, 2.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
